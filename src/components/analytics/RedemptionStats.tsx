  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        <div>
          <div className="mb-1 text-sm text-gray-400">Total Today</div>
          <div className="text-xl font-semibold text-white">{mockData.stats.total}</div>
        </div>
        <div>
          <div className="mb-1 text-sm text-gray-400">Average</div>
          <div className="text-xl font-semibold text-white">{mockData.stats.average}</div>
        </div>
        <div>
          <div className="mb-1 text-sm text-gray-400">Peak</div>
      ...
      <div className="relative h-32">
        {mockData.today.map((data, i) => (
          <div
            key={data.hour}
            className="absolute bottom-0 rounded-t-sm bg-blue-500 transition-all duration-500"
            style={{
          ...
          >
            <div className="absolute -top-5 left-1/2 -translate-x-1/2 transform text-xs text-gray-400">
              {data.count}
            </div>
          </div>
        ))}
